...
from .controller import Eg026Controller
...
def permissions_change_single_setting():
    ...
    # 1. Get required arguments
    args = Eg026Controller.get_args()
    try:
        # 2. Call the worker method to change a setting in an existing permission profile
        response, changed_settings = Eg026Controller.worker(args)
...
def get_view():
    ...
    permission_profiles = Eg026Controller.get_permissions_profiles(args)
    return render_template(
